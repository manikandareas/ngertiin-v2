...
Clerk production menggunakan `app-ngertiin.whoismanik.dev` sebagai secondary application. Lima CNAME dan domain sudah verified; endpoint HTTPS `/v1/environment` mengembalikan HTTP 200. Secret lengkap sudah divalidasi lewat Clerk Backend API dan dipasang di Dokploy. Halaman login production tampil di Brave. Google Sign-In masih gagal `Missing required parameter: client_id`: koneksi Google/GitHub belum diisi kredensial production. Form client baru sudah disiapkan, menunggu konfirmasi pembuatan dan pemasangan ke Clerk. Callback keduanya: `https://clerk.app-ngertiin.whoismanik.dev/v1/oauth_callback`.
...
Bucket private `ngertiin-backups` sudah dibuat. Token terbatas bucket tersebut sudah disiapkan pada form Cloudflare tetapi belum dibuat; destination dan jadwal Dokploy menunggu konfirmasi token. Jangan menganggap backup sudah aktif.

Login pengguna hingga alur source → modul → evaluasi, pemrosesan job provider/Firecrawl, streaming authenticated, restart recovery, serta backup/restore production: **NOT RUN**. Infrastruktur dan frontend sudah live, tetapi autentikasi sosial dan kesiapan penggunaan penuh belum selesai.
