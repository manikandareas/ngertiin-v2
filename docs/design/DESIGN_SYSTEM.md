...
Light, Duolingo-inspired foundation with Spark Blue (`#1cb0f6`) as the primary.
...
Dashboard uses a 256px desktop sidebar with a compact logo and collapse control at the top, text-first navigation with Hugeicons on the right, and a neutral pill for the active page. A double-chevron control collapses it to an 64px icon rail. Recent modules appear below a divider, grouped by their updated date (Hari ini, Kemarin, Sebelumnya); the list shares the dashboard query and links ready modules to their journey. Navigation uses compact 40px desktop rows with 14px labels. Recent module groups have 16px section spacing. The account avatar/name and theme control share a single footer row; module creation stays in the main navigation. On smaller screens navigation becomes a compact top bar. The shared AppShell applies this navigation to all existing pages using it. Dashboard content has its own scroll area with a large personalized greeting on a pale-blue highlight, followed by a muted subheading. Four cards show recently updated modules, the next learning step, current/best streak, and XP. The recent-module card introduces a pale yellow surface (#fffbe8, ink #785515) with a dark-theme counterpart. Cards stay aligned without rotation. Desktop typography uses a 36px greeting, 24px supporting question, 18px featured module title, 20px statistics, and 14px body text. Card padding is 24px, card gaps are 24px, and the module list has 32px of separation. The bottom module section uses inline rows with search and colorful status, progress, and duration badges. Titles and badges use a consistent 8px gap and wrap naturally, without growing title columns or a grid. Difficulty dots are blue for beginner, orange for intermediate, red for advanced, and neutral when unspecified; their accessible labels and hover titles explain the difficulty. Composer components stay mounted but hidden on the default dashboard, preserving drafts while toggling creation; explicit creation links reveal the inline composer through #module-composer. Empty, loading, and error states remain supported.

The shell uses an animated sun/moon button. Until an explicit choice exists, the theme follows device preference changes. Clicking toggles light/dark and persists the choice under `ngertiin-theme`. The animation follows the same `data-theme` selector as the palette and honors reduced motion. Dark semantic tokens use a neutral charcoal canvas (#141414), cards (#202020), and neutral gray borders with Spark Blue accents. Legacy page-specific hard-coded styles outside the dashboard still need dark-mode migration.
